...
impl Maze {
    ...
    fn apply_byte(&mut self, index: usize) {
        let x = self.byte_list[index].x;
        let y = self.byte_list[index].y;
        self.grid[y][x] = '#';
    }
    ...
    fn find_blocking_byte(&mut self) -> Result<Coordinates> {
        for i in 1024..self.byte_list.len() {
            self.apply_byte(i);
            self.score_grid = vec![vec![None; self.size]; self.size];
            let _ = self.create_score_grid();

            if let Some(_val) = self.score_grid[self.end.y][self.end.x] {
                continue;
            } else {
                println!("END FOUND AT INDEX {}", i);
                return Ok(self.byte_list[i]);
            }
        }
        Err(Error::msg("No blocking byte found"))
    }
}
...
fn main() -> Result<()> {
    ...
    let coord = maze.find_blocking_byte()?;

    let end = start.elapsed();
    println!("Total Score {:?} in {:?}", coord, end);
